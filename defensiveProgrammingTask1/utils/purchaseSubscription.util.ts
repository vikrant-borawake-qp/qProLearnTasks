...
export const validateInputs = (
  ...
  const { name, cardCvv, cardExpiry, cardNumber, subscriptionId, userId } =
  ...
  if (!subscriptionId) return "No subscription id provided";
  const subscription = this.subscriptionRepository.findById(subscriptionId);
  if (!subscription) return "Invalid subscription id";
